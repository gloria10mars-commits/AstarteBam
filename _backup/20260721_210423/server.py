...
ARGS_REQUIRED = VALID_ACTION_TYPES - {"list_dir"}
...
def validate_request(data):
    ...
    for index, action in enumerate(actions, 1):
        ...
        if action_type not in VALID_ACTION_TYPES:
            return False, prefix + ": type invalide {!r}".format(action_type)
...
def make_log(action_type, action_args, status, message="", data=None, error=None, duration_ms=0):
    entry = {
...
def execute_direct_request(data, request_id):
    ...
    for action in data["actions"]:
        ...
        add_log(request_id, make_log(action["type"], action.get("args", ""), "success" if result.get("ok") else "error", result.get("output") or result.get("error") or result.get("warning") or "", duration_ms=result["duration_ms"]))
...
def build_continue_prompt(entry):
    ...
    for i, r in enumerate(results):
        ...
        lines.append("{}/{} {} : {}".format(i + 1, len(results), atype, str(output)[:200]))
    lines.extend([
        "",
        "[INSTRUCTION]",
...
def build_retry_prompt(entry):
    ...
    for i, r in enumerate(results):
        ...
        lines.append("Action {}/{} : {} {} — {}".format(i + 1, len(results), atype, flag, str(detail)[:200]))
...
def extract_json_blocks(text):
    ...
    results = []
    import json, re
    # Stratégie 1: Markdown
    for m in re.finditer(r'```(?:json)?\s*([\s\S]*?)```', ctext):
        try:
            p = json.loads(m.group(1).strip())
            if 'actions' in p: results.append(p)
        except: pass
    # Stratégie 2: Braces
    if not results:
        try:
            s, e = ctext.find('{'), ctext.rfind('}')
            if s != -1 and e != -1:
                p = json.loads(ctext[s:e+1])
                if 'actions' in p: results.append(p)
        except: pass
    return results
...
def worker_process_prompt(entry):
    ...
    active_model = GROQ_MODEL if AI_PROVIDER == "groq" else NVIDIA_MODEL
...
def worker_process_prompt_collaborative(entry):
    ...
    executed_actions = [l for l in task_logs if l.get("status") in ("success", "error")]
