...
def execute_direct_request(data, request_id):
    ...
    for action in data["actions"]:
        ...
        add_log(request_id, make_log(action["type"], action.get("args", ""), "success" if result.get("ok") else "error", result.get("output") or result.get("error") or result.get("warning") or "", duration_ms=result["duration_ms"], max_msg=40000 if action.get("type") == "delegate" else 4000))
...
def worker_process_prompt_collaborative(entry):
    ...
    if executed_actions:
        ...
        for i, l in enumerate(executed_actions, 1):
            ...
            cap = 20000 if atype == "DELEGATE" else 200
