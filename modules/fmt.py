#!/usr/bin/env python3
"""Format Python file with autopep8 --max-line-length 100."""
...
def handle(args, cwd, stdin_data=""):
    """Format Python file with max line length 100. Args = file path."""
    if not args or not args.strip():
        return {"ok": False, "error": "Usage: fmt <filepath>"}
    ...
    try:
        result = subprocess.run(
            ["autopep8", "--in-place", "--max-line-length", "100", fpath],
            capture_output=True, text=True
        )
        if result.returncode == 0:
            return {
                "ok": True,
                "msg": "Formatted (max-line-length=100): {}".format(fpath),
                "file": fpath,
            }
        else:
            return {
