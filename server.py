...
try:
    ...
    # auto = router décide dual-expert même en mode individual ; off = strictement le mode choisi
    COORDINATION_MODE = os.environ.get("COORDINATION_MODE", "auto").strip().lower()
    if COORDINATION_MODE not in ("auto", "off", "force_dual"):
        raise ConfigError("COORDINATION_MODE doit être auto, off ou force_dual")
...
ACTION_FIELDS = {"type", "args", "content", "interactive", "path"}
...
def validate_request(data):
    ...
    for index, action in enumerate(actions, 1):
        ...
        if "path" in action and not isinstance(action["path"], str):
            return False, prefix + ": path doit être une chaîne"
...
_FEEDBACK_CAPS = {"DELEGATE": 30000, "READ_FILE": 20000, "LIST_DIR": 8000}
...
def extract_code_from_expert(text):
    """Extrait le code d'une reponse d'expert : le plus gros bloc markdown ``` s'il existe,
    sinon le texte brut. Utilise pour la delegation-vers-fichier."""
    if not text:
        return ""
...
def execute_action(action, cwd, request_id=None, log_index_offset=0):
    ...
    try:
        if atype == "delegate":
            target = args.lower()
            prompt = content
            add_log(request_id, make_log("system", "delegation", "running", f"Délégation interceptée vers {target}. Execution de l'agent secondaire..."))
            
            # Injection d'un prompt système surpuissant pour Gemini
            system_prompt = "Tu es l'Expert Frontend UI/UX d'AstarteBam. Ton rôle est de générer du code d'interface utilisateur (HTML, CSS, JS, etc.) qui est magnifique, moderne, ultra-ergonomique, avec de belles animations, et prêt pour la production. Tu as un grand sens esthétique. Ne donne aucune explication, réponds UNIQUEMENT avec les blocs de code complets."
            messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}]
            
            # Appel interne au provider cible
            res, slot, err = PROVIDER_REGISTRY.call(messages, preferred=target, strict=True)
            if err:
                return {"ok": False, "error": f"Échec de la délégation à {target}: {err}"}
            # Délégation-vers-fichier : si "path" est fourni, la réponse de l'expert
            # est écrite DIRECTEMENT dans le fichier (zéro retranscription par
            # l'orchestrateur -> ni troncature ni altération du code).
            dest = (action.get("path") or "").strip()
            if dest:
                code = extract_code_from_expert(res)
                wr = do_write_file(dest, code, cwd)
                if not wr.get("ok"):
                    return {"ok": False, "error": "Retour {} reçu mais écriture impossible dans {} : {}".format(target, dest, wr.get("error"))}
                preview = code if len(code) <= 25000 else code[:25000] + "\n... [aperçu tronqué — fichier COMPLET déjà sur disque ; utilise read_file si tu veux tout relire]"
                return {"ok": True, "output": "[RETOUR DE {}] : {} octets écrits directement dans {}. Le fichier est déjà sur disque : ne le réécris PAS. CONTENU PRODUIT PAR GEMINI (pour ta revue et ta synthèse finale) :\n{}".format(target.upper(), len(code), dest, preview)}
...
def process_prompt_with_actions(entry, action_data):
    ...
    if not valid:
        error_msg = f"ERREUR DE PARSING JSON : Ton JSON est invalide ou corrompu ({err}). RAPPEL : Réponds UNIQUEMENT avec le dictionnaire JSON, SANS AUCUN TEXTE AVANT NI APRÈS. Assure-toi que les guillemets et virgules sont corrects."
        add_log(entry["id"], make_log("orchestrator", "plan_retry", "error", error_msg))
    ...
    delegate_count = sum(1 for a in actions if a.get("type") == "delegate")
    if delegate_count > 1:
        entry["status"] = "error"
        err = f"INTERDIT: Tu as utilisé {delegate_count} actions 'delegate' dans un seul plan. Règle absolue: 1 SEUL DELEGATE par plan. Demande à Gemini de tout mettre dans un seul fichier."
        entry["error"] = err
        return False, err
...
def call_llm_api(messages, retries=2, preferred_override=None, strict=None):
    preferred = preferred_override if preferred_override else AI_PROVIDER
    # Délégation / dual-expert : strict par défaut si un provider est forcé
    if strict is None:
        strict = bool(preferred_override)
    log_activity("PROCESS", f"Appel IA ({preferred}, strict={strict})...")

    # PROVIDER_REGISTRY.call renvoie (text, slot, error)
    text, slot, error = PROVIDER_REGISTRY.call(
        messages,
        preferred=preferred,
        preferred_slot=AI_SLOT if ORCHESTRATION_MODE == "individual" else None,
        strict=strict,
    )
...
def worker_process_prompt_collaborative(entry):
    ...
    if executed_actions:
        ...
        history_str = "
--- [RAPPORT D'EXECUTION - TOUR " + str(tour) + "] ---
"
        ...
        for i, l in enumerate(executed_actions, 1):
            ...
            if l.get("status") == "success" and atype in ("WRITE_FILE", "DELEGATE", "REPLACE", "APPEND"):
                if args_lbl: done_files.add(args_lbl)
                
            message = str(l.get('message', '')).strip()
            summary = (args_lbl + (" → " if args_lbl and message else "") + message)
            
            if "RETOUR DE GEMINI" in summary and len(summary) > 700:
                summary = summary[:700] + "
... [CODE UI MASQUÉ POUR ÉVITER DE SATURER TA MÉMOIRE] ...
"
            else:
                cap = _FEEDBACK_CAPS.get(atype, 1000)
                summary = (summary[:cap] + "...") if len(summary) > cap else summary
                
            history_str += "[" + str(i) + "/" + str(len(executed_actions)) + "] " + atype + " | " + status_label + "
   ➤ Sortie : " + summary + "
"
        
        if done_files:
            history_str += "
[FICHIERS DÉJÀ CRÉÉS ET OPÉRATIONNELS (NE LES REFAIS PAS)]
"
            for df in sorted(list(done_files)):
                history_str += "- " + df + "
"
                
        history_str += "
[DIRECTIVES DE CONTINUATION OBLIGATOIRES]
1. ANALYSE les résultats ci-dessus.
2. SI TON PROJET EST ASSEMBLÉ ET QUE LES FICHIERS SONT CRÉÉS, TU DOIS OBLIGATOIREMENT T'ARRÊTER en renvoyant {\"version\":2,\"thought\":\"Terminé\",\"actions\":[]}
3. NE REFAIS SOUS AUCUN PRÉTEXTE une action marquée SUCCÈS.
4. CORRIGE les erreurs (❌) en priorité.
"
    log_activity("FEEDBACK", f"Historique compilé ({len(executed_actions)} actions) envoyé aux agents.")
    
    # -- Retroaction plan : un plan invalide n'est plus fatal --
    # L'erreur de validation est renvoyee a l'IA pour correction (3 essais max).
    plan = None
    plan_error = ""
    MAX_PLAN_TRIES = 3
    for plan_try in range(1, MAX_PLAN_TRIES + 1):
        prompt_used = entry.get("retry_prompt", entry["prompt"])
        if plan_error:
            prompt_used += (
                "\n\n[CORRECTION REQUISE - PLAN PRECEDENT REJETE]\n"
                "Ton plan JSON precedent a ete REJETE par le validateur : " + plan_error + "\n"
                "Renvoie un plan JSON COMPLET et VALIDE. Rappels critiques :\n"
                "- Types autorises : exec, read_file, write_file, write_file_b64, replace, append, delete_file, copy, list_dir, module, fix, upload, delegate\n"
                "- Pour 'replace', content DOIT contenir les 3 marqueurs : <<<<<<< SEARCH puis ======= puis >>>>>>> REPLACE\n"
                "- Champs autorises par action : type, path, args, content, interactive (rien d'autre)\n"
                "- Reponds UNIQUEMENT avec le bloc JSON du plan, sans texte autour."
            )
            add_log(entry["id"], make_log("orchestrator", "plan_retry", "info",
                "Plan invalide ({}) - nouvel essai {}/{} demande a l'IA.".format(plan_error, plan_try, MAX_PLAN_TRIES)))
        plan, report = orchestrator.run(entry["id"], prompt_used, task_history=history_str)
    ...
    if plan_error:
        entry["status"] = "error"
        entry["error"] = "Plan collaboratif invalide apres {} essais : {}".format(MAX_PLAN_TRIES, plan_error)
    ...
    while True:
        try:
            ...
            if WORKER_MODE == "tampermonkey":
                ...
                try:
                    use_collab = (ORCHESTRATION_MODE == "collaborative")
                    # Coordination auto : dual-expert même en mode individual
                    if COORDINATION_MODE == "force_dual":
                        use_collab = True
                    elif COORDINATION_MODE == "auto" and not use_collab:
                        try:
                            decision = IntelligentRouter().route(found.get("retry_prompt") or found.get("prompt") or "")
                            if decision.category == "dual_expert":
                                use_collab = True
                                add_log(found["id"], make_log(
                                    "orchestrator", "auto_dual", "running",
                                    "Coordination auto : dual-expert DeepSeek+Gemini activé."
                                ))
                        except Exception as route_exc:
                            log_activity("ROUTE", f"Router error: {route_exc}", level="WARN")
                    if use_collab:
...
def print_startup_summary():
    ...
    print("  Coord.   : {}".format(COORDINATION_MODE))
    print("  Coord.   : {}".format(COORDINATION_MODE))
    if WORKER_MODE == "api":
